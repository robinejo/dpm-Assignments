...
# data_demographics_raw_messy <- read_csv("../../implicit evaluations study/data/raw/data_demographics_raw_messy.csv", skip = 2) |>
#   janitor::clean_names()

# self report measure
data_selfreport_raw <- read_csv("../../implicit evaluations study/data/raw/data_selfreport_raw.csv") |>
  janitor::clean_names()
...
## Exclusions for self report (Assignment 7 Robine)
...
```{r}
data_selfreport_minus_NA <- data_selfreport_scored |>
  filter(positive != "other/missing/error",
         prefer != "other/missing/error")
```


# Affect Misattribution Procedure

## Exclusions for AMP (Assignment 7 Robine)

Add exclusions for the AMP task. It should have the "correct" number of trials. How to know what number is the correct number of trials?

```{r}
data_amp_how_many_trials <- data_amp_raw |>
  select("subject", "blockcode", "trialcode", "latency") |>
  filter(blockcode != "practice", 
         trialcode != "instructions")

data_amp_how_many_trials |>
  count(subject)

data_amp_minus_max_trial <- data_amp_how_many_trials |>
  filter(subject != 246532124, 504546409) #error

  
data_amp_performance_ok <- data_amp_performance_criteria |>
  filter(exclude_amp_performance != "exclude")


data_amp_nur_test <- data_amp_raw |> 
  filter(blockcode != "practice", 
         trialcode != "instructions")
```
